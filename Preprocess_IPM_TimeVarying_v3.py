...
Figsavedir = datapath+'figure_IPM109_MTX_cut100_HM_N5120_256\\'
#Figsavedir2 = datapath+'Img_IPM109timevarying_HM_N2560_256\\'
...
for cond_name in folder_list: # 상태
    ...
    class_num = class_total.index(cond_name.split('_')[-1])
    cond = class_total[class_num]
    for file_name in os.listdir(datadir+cond_name):
        ...
        # idx_samp_ start = list(range(0,log_df_len-N,N))#no overlap np.arange(2000, 4000 - 1, 1)
        overlap_n = 256#1280
        ...
        for n_i in range(Nlen-1):
           ...
           for pp in range(3):
                ...
                if not resampled_signal==[]:
                    ...
                    tanhX = minmax_scale(np.real(hilbert(resampled_signal)), feature_range=(-1, 1), axis=0,copy=True)  # scaler.fit_transform(temp_signal[:,b_n])
                    tanhY = minmax_scale(np.imag(hilbert(resampled_signal)), feature_range=(-1, 1), axis=0, copy=True)
                    ...
                    # if pp == 2:
                    #     Vizfig.suptitle(figname1 + '\n' + ' MTXscale ' + 'col abc phase', x=0.5, y=0.98)
                    #     Vizfig.tight_layout()
                    #     Vizfig.savefig(Figsavedir + 'MTX scale ' + plane_fig_name + '.png')
                    #     plt.close(Vizfig)
           ...
           if token==3:
                ...
                label_total = np.append(label_total,class_num)
           else:
                print('not included: '+plane_fig_name)
        label_total=label_total[1:];
        tanh_MTX_total=tanh_MTX_total[1:];#np.transpose(tanh_MTX_total[:,:,:,1:],(3,2,0,1));
        last_num = figname1[-1]#int(Imgname_split[-1].split('.')[0][0])
        if last_num ==3:
            last_num = 2
        savepath = './output/MTX_cut100_HM_N5120_256/'
        np.save(savepath+'MTX_data'+str(last_num)+'_Timevarying100_'+cond+str(class_num)+'.npy',tanh_MTX_total)
        np.save(savepath+'MTX_label'+str(last_num)+'_Timevarying100_'+cond+str(class_num)+'.npy',label_total)
