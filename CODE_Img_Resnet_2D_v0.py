...
tboardpath=datapath+'log\\'
writer = SummaryWriter(tboardpath+'IPM109Steady_SE_Resnet_experiment_221011_v1')
...
# Img_analytic_plane0 211227_IPM_BIF1_2250RPM_2.35N_constant_10Hz_1ni0.png
...
train_label=label[1:]
train_data=np.transpose(data[:,:,:,1:],(3,2,0,1))

test_label=label[1:]
test_data=np.transpose(data[:,:,:,1:],(3,2,0,1))

valid_label=label[1:]
valid_data=np.transpose(data[:,:,:,1:],(3,2,0,1))
...
TRAIN_DATASET=MotorDataset(train_x_sn,train_y_sn)
VALID_DATASET=MotorDataset(valid_x_sn,valid_y_sn)
TEST_DATASET=MotorDataset(test_x_sn,test_y_sn)
...
def modeltrain(model, criterion, optimizer, epochs, train_loader=train_loader, valid_loader=valid_loader):
    # 출력
    ...
    for e in range(1, epochs + 1):
        ...
        for X_train_batch, y_train_batch in train_loader:
            ...
            # _, y_pred_tags = torch.max(y_pred_softmax, dim=1)
            #
            # correct_pred = (y_pred_tags == y_train_batch.long()).float()
            # acc = correct_pred.sum() / len(correct_pred)
            #
            # acc = torch.round(acc * 1000) / 10
            # train_acc = acc
        ...
        with torch.no_grad():
            ...
            for X_val_batch, y_val_batch in valid_loader:
                ...
                # _, y_pred_tags = torch.max(y_pred_softmax, dim=1)
                #
                # correct_pred = (y_pred_tags == y_val_batch.long()).float()
                # acc = correct_pred.sum() / len(correct_pred)
                # acc = torch.round(acc * 1000) / 10
                # val_acc = acc
        ...
        if e == 0:
            torch.save(model.state_dict(), path + 'model\\' + conditionname+' model.pth')
            fin_epoch = e
